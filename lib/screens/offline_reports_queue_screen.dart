...
class _OfflineReportsQueueScreenState extends State<OfflineReportsQueueScreen> {
  ...
  void _listenToConnectivityChanges() {
    _connectivitySubscription = Connectivity().onConnectivityChanged.listen((result) {
      ...
      if (wasOffline && _isOnline && _queuedReports.isNotEmpty) {
        _autoSyncAll();
      }
    });
  }
  ...
  Future<void> _loadQueuedReports() async {
    // TODO: Load from local storage (SharedPreferences, Hive, etc.)
    print('Loading queued reports from local storage...');
      ...
      _queuedReports = [
        {
          ...
          'incidentTitle': 'Suspicious Person',
          'description': 'Individual loitering around school...',
        ...
        {
          'id': 'draft_002',
          'incidentType': 'vehicle_activity',
          'incidentTitle': 'Vehicle Activity',
          'description': 'Car circling the block repeatedly...',
          'status': 'failed',
  ...
  Future<void> _syncReport(String reportId) async {
    ...
    try {
      ...
      // await ReportService.uploadOfflineReport(report);
      ...
      setState(() {
        final index = _queuedReports.indexWhere((r) => r['id'] == reportId);
        if (index != -1) {
          _queuedReports[index]['status'] = 'synced';
        }
      });
      ...
      setState(() {
        final index = _queuedReports.indexWhere((r) => r['id'] == reportId);
        if (index != -1) {
          _queuedReports[index]['status'] = 'failed';
          _queuedReports[index]['retryCount'] = (_queuedReports[index]['retryCount'] ?? 0) + 1;
          _queuedReports[index]['errorMessage'] = e.toString();
        }
      });
  ...
  Future<void> _autoSyncAll() async {
    if (!_isOnline || _queuedReports.isEmpty) return;

    setState(() => _isSyncing = true);

    for (var report in _queuedReports) {
  ...
  void _deleteReport(String reportId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this queued report?'),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              setState(() {
                _queuedReports.removeWhere((r) => r['id'] == reportId);
              });
